...
//   "BO68kj27Xzwxq8FpHF69q7rz2h0L0VenRBYw_Xq-g1o25bUK2s8e3XOXqs4tUfE3Y6UF3aQlQwuGzfMjwQ9rqRw";
...
app.post("/register", (req, res) => {
  ...
  User.findOne({ user_id: user_id }).then((user) => {
    if (user) {
      res.render("login", { error_msg: "User Already Registered" });
    } else {
      const newUser = new User({firstName,lastName,gender,dob,contact,address,email,linkedin,user_id,
...
app.post("/admin/register", (req, res) => {
  ...
  Admin.findOne({ user_id: user_id }).then((admin) => {
    if (admin) {
      ...
      const newAdmin = new Admin({firstName,lastName,gender,dob,contact,address,email,user_id,password,
        password2,is_admin
      });
...
app.post("/taketest1", isLoggedIn, (req, res) => {
  Problem.find({ index: { $gt: 0, $lt: 11 } }, function (err, problem) {
    console.log(problem);
    let score = 0;
    if (req.body.op1 === problem[0].op) score += 10;
    if (req.body.op2 === problem[1].op) score += 10;
    if (req.body.op3 === problem[2].op) score += 10;
    if (req.body.op4 === problem[3].op) score += 10;
    if (req.body.op5 === problem[4].op) score += 10;
    if (req.body.op6 === problem[5].op) score += 10;
    if (req.body.op7 === problem[6].op) score += 10;
    if (req.body.op8 === problem[7].op) score += 10;
    if (req.body.op9 === problem[8].op) score += 10;
    if (req.body.op10 === problem[9].op) score += 10;
